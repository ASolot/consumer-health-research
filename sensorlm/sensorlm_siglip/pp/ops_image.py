...
@Registry.register("preprocess_ops.decode_sensor", replace=True)
@utils.InKeyOutKey()
def get_decode_sensor():
  """Decode an encoded sensor image string. Added by SensorLM.

  Args:

  Returns:
    The decoded image of sensor data.
  """

  def _decode(image):
    features = tf.io.parse_tensor(image, out_type=tf.double)
    features = tf.reshape(features, (1440, 26, 1))
